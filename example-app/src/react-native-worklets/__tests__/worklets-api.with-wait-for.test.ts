...
describe('runOnUI', () => {
  ...
  test('passes arguments to worklet', async () => {
    const worklet = jest.fn();
  ...
  test('multiple independent calls each schedule independently', async () => {
    const worklet = jest.fn();
    runOnUI(worklet)(1);
    runOnUI(worklet)(2);
    await new Promise(r => setTimeout(r, 0));
...
describe('runOnUIAsync', () => {
  ...
  test('Promise resolves with worklet return value', async () => {
    const resolved = await runOnUIAsync((n: number) => n * 2)(7);
    expect(resolved).toBe(14);
  });

  test('multiple concurrent calls resolve independently', async () => {
    const [r1, r2] = await Promise.all([
      runOnUIAsync((n: number) => n + 1)(10),
      runOnUIAsync((n: number) => n + 1)(20),
    ]);
    expect(r1).toBe(11);
    expect(r2).toBe(21);
  });
...
describe('scheduleOnUI', () => {
  test('fires worklet with args after timer flush', async () => {
    const worklet = jest.fn();
